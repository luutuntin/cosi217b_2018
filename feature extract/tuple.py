#	-*- coding:utf-8 -*-
import xlwt
import xlrd
import csv
import re
import pickle

class subGraph():
	def __init__(self, graph, articleId, sentenceId, annotation):
		self.graph = graph
		self.articleId = articleId
		self.sentenceId = sentenceId
		self.annotation = annotation

	def __repr__(self):
		return "{}\t{}\t{}\t{}".format(self.articleId, self.sentenceId, self.graph, self.annotation)



def main():
	subgraphFilePath = "./PART-amr-release-1.0-training-proxy-subgraph.txt"
	annotationFilePath = "./annotation.xlsx"
	lino = 0
	subGraphs = []

	subgraph = open(subgraphFilePath,'r').read().split('-'*50+'\n')[2:]
	annotation = xlrd.open_workbook(annotationFilePath).sheets()[0]
	maxrows = annotation.nrows
	for s in subgraph:
		line = s.split('\n')
		index = re.search("id\s(\S+)\.(\d+)",line[0])	#/d can only match one digit
		aid = index.group(1)
		sid = index.group(2)
		for i in range(len(line)):
			if line[i].startswith('#'):
				continue
			else:
				break
		sg = '\n'.join(line[i:])
		if not sg:
			continue
		sg = sg.split('\n\n')[:-1]


		for e in sg:
			r = re.search("\((\d+)\)",e)
			if not r:
				print('sb')
			ind = int(r.group(1)) 	# a integer
			graph = e[r.end(1)+1:]
			try:
				while lino < maxrows and annotation.row(lino)[1].value != aid+'.'+sid:
					#print(annotation.row(lino)[1].value)
					lino += 1

				i = 1
				while lino + i < maxrows and ind != annotation.row(lino+i)[2].value:
					#print(annotation.row(lino+i)[2].value)
					i += 1
				if annotation.row(lino+i)[3].value == 'y' or annotation.row(lino+i)[3].value == 'yes':
					y = 1.0
				else:
					y = 0.0
			except IndexError:
				print(aid+'.'+sid)
			subGraphs.append(subGraph(graph,aid,sid,y))
	# print(subGraphs)
	with open('./tuple.pkl','wb') as p:		#the output file need to be open as wb if list
		pickle.dump(subGraphs,p)

if __name__ == '__main__':
	main()


