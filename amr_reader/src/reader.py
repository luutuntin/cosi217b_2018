...
from amr_reader.models.Node import Node
...
# https://github.com/luutuntin/Cinderella/blob/master/amr_reader/src/reader.py

def main_amr_table(raw_amrs):
    ''' 
     Main_amr_table function
     Input: raw AMR
     Output: 'amr_table'
             container: dict()   dict()      Sentence
             #key:       docid -> senid -> sentence object
             key:       docid -> int_id -> sentence object
    '''
    
    amr_table = dict()
#    sentences = raw_amrs.strip().split('# ::id ')

    for i in re.split('\n\s*\n', raw_amrs):
#        if i == '': continue
        if not i.startswith("# ::id"): continue
        sentid = re.search('::id (.*?) ', i).group(1)
        docid = sentid[:sentid.rfind('.')]
        ### create int_id for indexing sentences in document of amr_table
        int_id = int(sentid[len(docid)+1:])
        sent = re.search('::snt (.*?)\n', i).group(1)       
...
# https://github.com/luutuntin/Cinderella/blob/master/amr_reader/src/amr.py
'''
 Return amr_table
'''

import os
# https://github.com/luutuntin/Cinderella/blob/master/amr_reader/src/raw.py
#import raw
from . import raw
#import reader
#import ne
...
#    amr_table = reader.main(amr)
    amr_table = main_amr_table(amr)
#    ne.add_named_entity(amr_table)
    ne.add_named_entity_amr_table(amr_table)
